...
def basic_geometric_programming(min_entropy, max_entropy):
  ...
  for _ in range(obj_terms):
    coeff = random.randint(1, coeff_max)
  ...
  for _ in range(num_constraints):
    ...
    used = set()
    for _ in range(terms_per_con):
      coeff = random.randint(1, min(5, coeff_max))

      while True:
        exponents = tuple(np.random.randint(0, exp_max + 1, size=n))
        if all(e == 0 for e in exponents):
          continue
    ...
    bound = round(expr_val * random.uniform(*tightness), 2)
  ...
  template = random.choice([
      ...
      "{bounds}, with x_i > 0. What is the minimum value?\n"
